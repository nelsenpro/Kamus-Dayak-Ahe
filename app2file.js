...
async function loadKamus() {
  ...
  try {
    const files = ['data-max.json', 'data-min.json', 'data-medium.json'];
    const responses = await Promise.all(files.map(file =>
      fetch(file + '?v=' + Date.now()).then(res => res.json())
    ));
    kamusMap = new Map();

    responses.forEach(response => {
      const { kamus } = response;
      if (kamus && kamus['id-dayak']) {
        Object.entries(kamus['id-dayak']).forEach(([key, value]) => {
          kamusMap.set(key, value);
        });
      }
    });
    setDefaultMessage();
